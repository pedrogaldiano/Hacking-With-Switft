...
let hints = [
NSLocalizedString("When naming the [variable](glossary://variable) in your `for`-`in` loop, remember to use [camelCase](glossary://camelCase), starting the variable with a lowercase letter. For example, `column`, `currentColumn`, and `eachColumn` all work as variable names. `Column` doesn’t work because it starts with a capital letter.", comment:"Hint"),

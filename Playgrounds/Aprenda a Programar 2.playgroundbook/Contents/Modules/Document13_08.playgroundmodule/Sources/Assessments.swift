...
let success = NSLocalizedString("### Whoa, that was awesome! \n\nDid you notice how the [index out of range error](glossary://index%20out%20of%20range%20error) was prevented? Say, for example, that the number of values in your array is `10`-that means the highest [index](glossary://index) you can access is `9` (because the index starts at `0`).\n\nIf your `index` variable is equal to `heights.count` (value `10`), you’ll try to access index `10`, which is outside the boundary of the array. Instead, you can reset the value of your `index` variable to `0` when it’s equal to the number of items in your array. \n\n[**Next Page**](@next)", comment:"Success message")
let hints = [
    NSLocalizedString("First, add a set of [Int](glossary://Int) values to your `heights` array. If you want to quickly add more items to the array, try pressing the square brackets and dragging to the right.", comment:"Hint"),

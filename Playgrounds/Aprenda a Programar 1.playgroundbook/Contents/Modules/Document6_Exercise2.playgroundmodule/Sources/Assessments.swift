...
public func assessmentPoint() -> AssessmentResults {
    ...
    var hints = [
        NSLocalizedString("Just like in the previous puzzle, you need to carry out a set of actions while a certain condition is true.", comment:"Hint"),
    ...
    if !checker.didUseWhileLoop {
        ...
        hints[0] = NSLocalizedString("Add a `while` loop by selecting `while` in the shortcut bar. Then add a condition that moves forward until reaching the end of the third platform.", comment:"Hint")
    ...
    for element in contents {
        if !element.contains("!isBlocked") && checker.didUseWhileLoop {
            ...
            hints[0] = NSLocalizedString("Each time your loop runs, you need to check whether your character is on a closed switch. If so, toggle the switch. Be careful not to toggle any open switches.", comment:"Hint")
            
        } else if element.contains("!isBlocked") && checker.didUseWhileLoop && checker.didUseConditionalStatement {
            success = NSLocalizedString("### Incredible work! \nUsing `while` loops is already making your code simpler. By pairing a `while` loop and [conditional code](glossary://conditional%20code), you can write code that’s much more adaptable. \n\n[**Next Page**](@next)", comment:"Success message")

...
public func assessmentPoint() -> AssessmentResults {
    ...
    let success = NSLocalizedString("### Super impressive! \nYou’ve tweaked an [algorithm](glossary://algorithm) to make it work in different circumstances. This is the power of code—creating programs that solve a problem in many different situations. For example, search engine algorithms can filter over a billion websites to give you the results that you’ll be most interested in, no matter what you search for. \n\n[**Next Page**](@next)", comment:"Success message")
    var hints = [
            NSLocalizedString("Take the existing code and see if you can make it work to solve this slightly different puzzle. Notice that you now have to take a different action when blocked both in front and on the right, instead of just being blocked on the right.", comment:"Hint"),
